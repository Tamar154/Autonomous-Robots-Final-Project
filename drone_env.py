...
class DroneEnv(gym.Env):
    def __init__(self):
        ...
        # Observation space: position (x, y, z) and distance to obstacles in six directions
    ...
    def _get_distance_to_obstacles(self):
        # Simulated sensor data for distance to obstacles in six directions (front, back, left, right, up, down)
    ...
    def _is_done(self, obs):
        distance_to_start = np.linalg.norm(obs[:3] - np.array([self.starting_position.x_val, 
                                                               self.starting_position.y_val, 
                                                               self.starting_position.z_val]))
        return distance_to_start < 1.0  # Close enough to the starting point
